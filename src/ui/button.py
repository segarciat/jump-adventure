...
class Button(AnimatedSprite):
    ...
    def __init__(self, action: typing.Callable, text: str, size: int, color, all_groups):
        ...
        """
        frame_info = [
            {'animation_number': Button._HOVER_OFF, 'image_names': ["blue_button04.png"]},
            {'animation_number': Button._HOVER_ON, 'image_names': ["blue_button02.png"]},
            {'animation_number': Button._CLICKED, 'image_names': ["blue_button03.png"]}
        ]
        AnimatedSprite.__init__(self, frame_info, all_groups)
        # Add text to buttons.
        for animation, images in self._images.items():
            for image in images:
